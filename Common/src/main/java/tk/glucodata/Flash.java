...
public class Flash {
    ...
    static volatile int remainingFlashes = -1; // -1 means unlimited

    static void start(Context context, long period) {
        start(context, period, -1); // -1 = unlimited flashes
    }

    static void start(Context context, long period, int flashCount) {
        dostop(); // Prevent zombie schedules: Stop any existing flash first
        ...
        stopflash = false;
        onflag = false; // Reset to ensure we start with ON
        remainingFlashes = flashCount; // -1 for unlimited, or specific count

        // Ensure safe minimum period
        if (period < 50)
            period = 50;
        flashschedule = scheduler.scheduleAtFixedRate(() -> {
            if (stopflash)
                return;
            var wasflash = flash;
            if (wasflash != null) {
                if (onflag) {
                    wasflash.off();
                    // Count only when turning off (one complete on+off cycle)
                    if (remainingFlashes > 0) {
                        remainingFlashes--;
                        if (remainingFlashes <= 0) {
                            stopflash = true;
                            if (flashschedule != null) {
                                flashschedule.cancel(false);
                            }
                        }
                    }
                } else {

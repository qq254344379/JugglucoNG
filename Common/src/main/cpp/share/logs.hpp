...
#endif
#endif
// #define LOGGER(...)  { fprintf(errorlogfile,__VA_ARGS__);
// fflush(errorlogfile); }
#include <errno.h>
#include <fcntl.h>
#include <stdarg.h>
...
loggert(const char *format, ...) __attribute__((format(printf, 1, 2)));

extern int vloggert(const char *format, va_list args)
...
#else
#ifdef __cplusplus
#define LOGGER(pformat, ...)                                                   \
  logprint("%lu %d " pformat, ::time(nullptr), getTid(), __VA_ARGS__)
#define LOGSTRING(pformat)                                                     \
  logprint("%lu %d " pformat, ::time(nullptr), getTid())
#else
#define LOGGER(pformat, ...)                                                   \
  logprint("%lu %d " pformat, time(NULL), getTid(), __VA_ARGS__)
...
#ifdef __cplusplus
template <class T, size_t N> constexpr void LOGAR(const T (&array)[N]) {
  LOGGERN(array, N - 1);
}
#else

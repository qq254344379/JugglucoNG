...
void nfcdata::showhistory() {
  ...
  int mostRecentHistoryAgeInMinutes =
      3 + (sensorAgeInMinutes - 3) % historyIntervalInMinutes;
  long onmindate = (gettime() + 30) / 60;
  for (int counter = 0; counter < history::num; counter++) {
    int glucoseLevelRaw = gethistoryglucose(counter);
    // skip zero values if the sensor has not filled the ring buffer yet
    // completely
    if (glucoseLevelRaw > 0) {
      int dataAgeInMinutes =
          mostRecentHistoryAgeInMinutes +
          (history::num - (counter + 1)) * historyIntervalInMinutes;
...
time_t totime_t(std::string_view inp) {
  ...
  const char *name =
      inp.data() + ((pos == std::string_view::npos) ? 0 : pos + 1);
  // const char *end = inp.data()+inp.length();
  const char *end = inp.end();
  const char *num =
      std::find_if_not(name, end, [](char val) { return isdigit(val) == 0; });
  if (end == num) {
...
int main(int argc, char **argv) {
  if (argc < 2) {
    cout << "Usage: " << argv[0] << " filenames" << endl
         << "Displays content of freelibre nfc data" << endl;
    return 2;
  }
  for (int i = 1; i < argc; i++) {
    ...
    uint8_t *mmapbuf = (uint8_t *)mmap(NULL, mmaplen, PROT_READ | PROT_WRITE,
                                       MAP_SHARED, fp, 0);
    close(fp);

...
#include "logs.hpp"
// #define LOGGER(...)  __android_log_print(ANDROID_LOG_VERBOSE, LOG_TAG,
// __VA_ARGS__)
...
extern std::pair<const ScanData *, const ScanData *>
makecalibrated(const SensorGlucoseData *sens, const ScanData *input,
               ScanData *calibrated, int nr, bool allvalues);
extern std::pair<const ScanData *, const ScanData *>
makecalibratedback(const SensorGlucoseData *sens, const ScanData *input,
                   ScanData *calibrated, int nr, bool allvalues);
...
void cpcolors(NVGcolor *foreground) {
  ...
  for (int i = 1; i < wholes; i++)
    memcpy(foreground + i * oldnrcolors, foreground,
           oldnrcolors * sizeof(foreground[0]));
  if (int left = nrcolors % oldnrcolors) {
    memcpy(foreground + wholes * oldnrcolors, foreground,
           left * sizeof(foreground[0]));
  }
}
...
static enum FontType { CHINESE, HEBREW, REST } chfontset = REST;
...
void JCurve::initfont(NVGcontext *avg) {
  ...
  thevg = avg;
  if (usedtext == &zhtext) {
    if (-1 == (font = whitefont = blackfont =
                   nvgCreateFont(avg, "dance-bold",
#ifdef JUGGLUCO_APP
...
#ifdef JUGGLUCO_APP
      if (-1 == (font = whitefont = blackfont = nvgCreateFont(
                     avg, "dance-bold", fontpath "DroidSansFallback.ttf"))) {
    ...
    if (-1 == (menufont = nvgCreateFont(avg, "regular",
      ...
      if (-1 == (menufont = nvgCreateFont(avg, "regular",
        ...
        if (-1 == (menufont = nvgCreateFont(avg, "regular",
                                            ...
                                            "truetype/DroidSansFallback.ttf"
#endif
                                            ))) {
    ...
    if (hebrew()) {
      auto fallback =
          nvgCreateFont(avg, "dance-bold", fontpath "DroidSans.ttf");

      font = whitefont = blackfont = nvgCreateFont(
          avg, "dance-bold", fontpath "NotoSansHebrew-Regular.ttf");
      nvgAddFallbackFontId(avg, font, fallback);

      //    auto menufallback = nvgCreateFont(avg, "regular",fontpath
      //    "NotoSerif.ttf");

#ifdef JUGGLUCO_APP
      menufont = nvgCreateFontMem(avg, "regular", (unsigned char *)fontfile,
                                  sizeof(fontfile), 0);
      int fallback2 =
          nvgCreateFont(avg, "regular", fontpath "NotoSerifHebrew-Regular.ttf");
...
#endif
    {
      ...
      if ((whitefont = nvgCreateFont(avg, "dance-bold",
                                     ...
                                     "roboto-fontface/roboto/Roboto-Regular.ttf"
//"/usr/share/fonts/truetype/noto/NotoSerif-Regular.ttf"
//"/usr/local/Wolfram/Wolfram/14.2/SystemFiles/Fonts/TrueType/Roboto-Regular.ttf"
...
#endif
#ifdef JUGGLUCO_APP
      constexpr const char menufonts[][sizeof(fontpath
                                              "SourceSansPro-SemiBold.ttf")] = {
          fontpath "Roboto-Medium.ttf",  fontpath "SourceSansPro-SemiBold.ttf",
          fontpath "NotoSerif.ttf",      fontpath "SourceSansPro-Regular.ttf",
          fontpath "Roboto-Regular.ttf", fontpath "DroidSans.ttf"};
      int fallback;
  ...
  bounds_t bounds;
  nvgTextBounds(avg, 0, 0, smaller, smaller + sizeof(smaller) - 1,
                bounds.array);
  ...
  nvgTextBounds(avg, 0, 0, timestring, timestring + sizeof(timestring) - 1,
                bounds.array);
  ...
  nvgTextBounds(avg, 0, 0, listitem, listitem + sizeof(listitem) - 1,
                bounds.array);
  ...
  nvgTextBounds(avg, xhalf, yhalf, exampl, exampl + sizeof(exampl) - 1,
                (float *)&sensorbounds);
...
pair<const ScanData *, const ScanData *> getScanRange(const ScanData *scan,
                                                      const int len,
...
/*
extern std::vector<pair<const ScanData*,const ScanData*>>
getsensorranges(uint32_t start,uint32_t endt) ;

std::vector<pair<const ScanData*,const ScanData*>> getsensorranges(uint32_t
start,uint32_t endt) { auto hists= sensors->sensorsInPeriod(start,endt) ;
...
std::vector<pair<const ScanData *, const ScanData *>>
getsensorranges(uint32_t start, uint32_t endt, bool calibrated, bool allvalues,
                bool calibratePast,
  ...
  for (int i = hists.size() - 1; i >= 0 && timeiter < endt; i--) {
    ...
    if (ran.first == ran.second)
      continue;
    for (const ScanData *striter = ran.second - 1; striter >= ran.first;
         striter--) {
      if (striter->valid()) {
        timeiter = striter->t;
        ran.second = striter + 1;
...
pair<const ScanData *, const ScanData *> getScanRangeRuim(const ScanData *scan,
                                                          const int len,
                                                          const uint32_t start,
                                                          const uint32_t end) {
...
/*
pair<const ScanData*,const ScanData*> getScanRangeRuim(const ScanData
*scan,const int len,const uint32_t start,const uint32_t end) { auto [low,high]=
getScanRange(scan,len,start,end); const ScanData *endscan= scan+len;
...
void JCurve::sidenum(NVGcontext *avg, const float posx, const float posy,
                     const char *buf, const int len, const bool hit) {
...
bool JCurve::glucosepointinfo(NVGcontext *avg, time_t tim, uint32_t value,
  ...
  if ((!selshown && nearby(posx - tapx, posy - tapy, density))) {
    ...
    //        int len=snprintf(buf,maxbuf,"%02d:%02d",
    //        tms->tm_hour,mktmmin(tms));
...
bool JCurve::glucosepoint(NVGcontext *avg, time_t tim, uint32_t value,
                          float posx, float posy) {
...
void JCurve::showScan(NVGcontext *avg, const ScanData *low,
                      const ScanData *high, const TX &transx, const TY &transy,
...
template <class TX, class TY>
void JCurve::showlineScan(NVGcontext *avg, const ScanData *low,
                          const ScanData *high, const RawData *rawLow,
                          const TX &transx, const TY &transy,
                          const int colorindex, bool search, bool useRaw,
                          bool forceGrey) {
#ifdef SI5MIN
  ...
  if (search) {
    nvgBeginPath(avg);
    nvgStrokeColor(avg, *getyellow());
    nvgFillColor(avg, *getyellow());
  ...
  for (const ScanData *it = low; it != high; it++) {
    if (it->valid()) {
      ...
      /*#ifndef NOLOG
      time_t ttim=tim;
                  LOGGER("showlineScan posx=%f tim=%ud
      %s",posx,tim,ctime(&ttim)); #endif */
...
pair<int32_t, int32_t> histPositions(const SensorGlucoseData *hist,
...
template <class TX, class TY>
void JCurve::histcurve(NVGcontext *avg, const SensorGlucoseData *hist,
                       const int32_t firstpos, const int32_t lastpos,
                       const TX &xtrans, const TY &ytrans,
                       const int colorindex) {
...
pair<float, float> JCurve::drawtrender(NVGcontext *avg,
                                       const std::array<uint16_t, 16> &trend,
  ...
  const auto gety = [y, h, mid](const short val) -> float {
    return y + h / 2.0 - (((val - mid) / hglurange) * h);
  };
...
pair<int, int>
JCurve::getextremes(const vector<int> &hists,
                    const pair<const ScanData *, const ScanData *> **scanranges,
                    int scannr, const pair<int32_t, int32_t> *histpositions) {
  int gmax = 0;
  int gmin = 6000;
  ...
  for (int i = 0; i < histlen; i++) {
    ...
    if (!his->isDexcom() ||
      ...
      for (auto pos = histpositions[i].first, last = histpositions[i].second;
           pos <= last; pos++) {
    ...
    for (int j = 0; j < scannr; j++) {
      ...
      for (const ScanData *it = srange[i].first, *last = srange[i].second;
           it < last; it++) {
        if (it->valid()) {
          auto mgdL = it->g;
          int glu = mgdL * 10;
...
template <class LT>
void JCurve::glucoselines(NVGcontext *avg, const float last,
                          const float smallfontlineheight, const int gmax,
                          const LT &transy, bool showlevels) {
...
//    LOGGER("glucoselines: unit=%f unit2=%f step=%d (%g) startl=%d
...
template <class LT>
const displaytime
JCurve::getdisplaytime(const uint32_t nu, const uint32_t starttime,
                       const uint32_t endtime, const LT &transx) {
...
static bool timemiddle() { return false; }
#ifdef NOCUTOFF
static bool nocutoff = true;
#endif
template <class LT>
void JCurve::timelines(NVGcontext *avg, const displaytime *disp,
                       const LT &transx, uint32_t nu) {

  const uint32_t tstep = disp->tstep;
  const uint32_t first = disp->first;
  const uint32_t last = disp->last;
#ifdef WEAROS
  const uint32_t numlast =
      (disp->last > nu) ? (disp->last - tstep) : disp->last;
#endif
...
template <class LT>
void JCurve::epochlines(NVGcontext *avg, uint32_t first, uint32_t last,
                        const LT &transx) {
...
void setmaxsensors(size_t sensornr) { setusedsensors(); }
...
#ifdef WEAROS
  len += sprintf(buf + len, " %s %02d %s", usedtext->daylabel[stmbuf.tm_wday],
                 stmbuf.tm_mday, usedtext->monthlabel[stmbuf.tm_mon]);
#else
  len += sprintf(buf + len, " %s %02d %s %d",
                 usedtext->daylabel[stmbuf.tm_wday], stmbuf.tm_mday,
                 usedtext->monthlabel[stmbuf.tm_mon], 1900 + stmbuf.tm_year);
#endif
  return len;
}

void JCurve::showbluevalue(NVGcontext *avg, const time_t nu, const int xpos,
                           std::vector<int> &used) {
  ...
  if (const auto *sens = sensors->getSensorData()) {
    if (!(sens->isDexcom() || sens->isSibionics()) || !sens->unused()) {
      if (time_t enddate = sens->expectedEndTime()) {
        ...
        memcpy(head, usedtext->sensorexpectedend.data(),
               usedtext->sensorexpectedend.size());
        ...
        nvgText(avg, -dheight / 2 + down - smallfontlineheight, dwidth - timex,
  ...
  if (settings->data()->IOB) {
    ...
    nvgTextAlign(avg, NVG_ALIGN_CENTER | NVG_ALIGN_BOTTOM);
    nvgText(avg, -dheight * .40f + down - smallfontlineheight,
            dwidth * .984f - timex, tbuf, tbuf + len);
    nvgResetTransform(avg);
  }
#endif
  ...
  if (showcurrentdate) {
    ...
    nvgTextAlign(avg, NVG_ALIGN_LEFT | NVG_ALIGN_TOP);
    {
      ...
      const float timex =
          portrait ? final_getx - timelen * 0.5f
                   : // Center in portrait top panel? Actually getx is
                     // calculated relative to xpos.
      ...
      if (settings->data()->IOB) {
        ...
        nvgText(avg, timex, 2 * smallfontlineheight + statusbarheight, tbuf,
                tbuf + len);
      }
#endif

      LOGGER("xpos=%d dwidth=%.1f headsize=%.1f density=%.1f getx=%.1f "
             "timex=%.1f\n",
...
void JCurve::showsavedomain(NVGcontext *avg, const float last, const float dlow,
                            const float dhigh) {
  nvgBeginPath(avg);
...
void JCurve::showunsaveredline(NVGcontext *avg, const float last,
                               const float dlow) {
  nvgBeginPath(avg);
...
void JCurve::showsaverange(NVGcontext *avg, const float last, const float dlow,
                           const float dhigh) {
  showsavedomain(avg, last, dlow, dhigh);
  showunsaveredline(avg, last, dlow);
}

void JCurve::showdates(NVGcontext *avg, time_t nu, uint32_t starttime,
                       time_t endtime) {
  LOGGER("duration=%d\n", duration);
  ...
  if (timdis > 0 && ((duration / timdis) < grens)) {
    ...
    timelen = sprintf(tbuf, "%02d-%02d-%d", stm->tm_mday, stm->tm_mon + 1,
                      1900 + stm->tm_year);
    ...
    timelen = sprintf(
        tbuf, "%s %02d-%02d-%d", usedtext->daylabel[tmbufstart.tm_wday],
        tmbufstart.tm_mday, tmbufstart.tm_mon + 1, 1900 + tmbufstart.tm_year);
...
#define equalday(x) (tmbufend.x == tmbufstart.x)
      if (!(equalday(tm_wday) && equalday(tm_mday) && equalday(tm_mon) &&
            equalday(tm_year))) {
        timelen = sprintf(
            tbuf, "%s %02d-%02d-%d", usedtext->daylabel[tmbufend.tm_wday],
            tmbufend.tm_mday, tmbufend.tm_mon + 1, 1900 + tmbufend.tm_year);
...
int JCurve::displaycurve(NVGcontext *avg, time_t nu) {
  ...
  hidden.clear();
  hists.clear();
  sensors->sensorsInPeriod(hists, starttime2, endtime,
                           [this, starttime2](const SensorGlucoseData *s) {
                             if (s->hide) {
  ...
  histpositions =
      new std::remove_reference_t<decltype(histpositions[0])>[histlen];
#ifdef SI5MIN
  ...
  for (int i = histlen - 1; i >= 0; --i) {
    ...
    // if(showscans)
    {
      ...
      scanranges[i] =
          getScanRange(scan.data(), scan.size(), starttime2, endtime);
    }
    // if(showstream)
    {
      scan = his->getPolldata();
      pollranges[i] =
          getScanRangeRuim(scan.data(), scan.size(), starttime2, endtime);
  ...
  if (showcalibrated) {
    auto califunc =
        settings->data()->CalibratePast ? makecalibratedback : makecalibrated;
    for (int i = histlen - 1; i >= 0; i--) {
      ...
      if (const int nr = pollranges[i].second - pollranges[i].first; nr > 0) {
        ...
        caliSpans[i] =
            califunc(sens, pollranges[i].first, calidata, nr, allvalues);
      } else {
        caliSpans[i] = {};
      }
    }
  ...
  const pair<const ScanData *, const ScanData *> *scanpoll[] = {
      scanranges, pollranges, caliSpans};
  LOGAR("Before getextremes");
  if ((setend < starttime2 || settime >= endtime)) {
    auto extr =
        getextremes(hists, scanpoll, std::size(scanpoll), histpositions);
    for (int i = 0; i < numdatas.size(); i++) {
  ...
  showsaverange(avg, dlast, transy(settings->targetlow()),
                transy(settings->targethigh()));
  ...
  glucoselines(avg, dlast, smallfontlineheight, gmax, transy,
               (starttime2 + duration / 3) < nu);

  //        nvgCircle(avg, posx,posy,foundPointRadius);

  LOGAR("before showhistories");
  const int colorsleft = nrcolors - catnr;
  const auto segcolor = [catnr, colorsleft,
                         colorseg = colorsleft / 4](int index, int seg) {
  ...
  if (showhistories) {
    ...
    for (int i = histlen - 1; i >= 0; i--) {
      ...
      histcurve(avg, sensors->getSensorData(index), histpositions[i].first,
                histpositions[i].second, transx, transy, colorindex);
    }
  }
  ...
  if (showcalibrated) {
    ...
    for (int i = histlen - 1; i >= 0; i--) {
      ...
      /*            const int nr=pollranges[i].second-pollranges[i].first;
                  ...
                  const auto *sens=sensors->getSensorData(index);
                  span<ScanData>
         cali=makecalibrated(sens,pollranges[i].first,calibrated,nr);*/
#ifdef JUGGLUCO_APP
      bool search = calibratedStreamsearchtype ==
                    (calibratedStreamsearchtype & searchdata.type);
#else
      bool search = false;
#endif
      const auto cali = caliSpans[i];
      if (cali.second > cali.first) {
        showlineScan(avg, cali.first, cali.second, nullptr, transx, transy,
                     colorindex, search, false, false);
      }
    }
  }
  ...
  if (showstream) {
    ...
    for (int i = histlen - 1; i >= 0; i--) {
      ...
      const RawData *rawLow =
          rawBase ? (rawBase + (pollranges[i].first - sens->getPollsData()))
                  : nullptr;

      int viewMode = sens ? sens->getinfo()->viewMode : 0;

      if (viewMode == 1) {
        showlineScan(avg, pollranges[i].first, pollranges[i].second, rawLow,
                     transx, transy, colorindex, search, true, false);
      } else if (viewMode == 3) {
        showlineScan(avg, pollranges[i].first, pollranges[i].second, rawLow,
                     transx, transy, colorindex, search, true, false);
        showlineScan(avg, pollranges[i].first, pollranges[i].second, rawLow,
                     transx, transy, colorindex, search, false, true);
      } else {
        showlineScan(avg, pollranges[i].first, pollranges[i].second, rawLow,
                     transx, transy, colorindex, search, false, false);
        if (viewMode == 2) {
          showlineScan(avg, pollranges[i].first, pollranges[i].second, rawLow,
                       transx, transy, colorindex, search, true, true);
        }
      }
    }
  }
  ...
  if (showscans) {
    for (int i = histlen - 1; i >= 0; i--) {
      ...
      showScan(avg, scanranges[i].first, scanranges[i].second, transx, transy,
               colorindex);
    }
  }
...
void JCurve::setlocale(NVGcontext *avg, const char *localestrbuf,
                       const size_t len) {
  LOGGER("locale=%s\n", localestrbuf);
...
Numdata *getherenums() { return newhit.numdisplay; }
Num newnum;
#include "numhit.hpp"
...
int64_t openNums(std::string_view numpath, int64_t ident) {
  ...
  NumDisplay *numdata =
      NumDisplay::getnumdisplay(index, numpath, ident, nummmaplen);
  if (numdata) {
...
void JCurve::showvalue(NVGcontext *avg, const ScanData *poll,
                       const SensorGlucoseData *hist, float getx, float gety,
                       int index, uint32_t nu) {
  if (!poll)
    return;
...
void JCurve::showlastsstream(NVGcontext *avg, const time_t nu, const float getx,
                             std::vector<int> &used) {
...
#ifdef JUGGLUCO_APP
  int success = false;
  bool neterror = false, usebluetoothoff = false, bluetoothoff = false,
       otherproblem = false;
  int blueperm = 2;
  ...
  for (int i = 0; i < usedsize; i++) {
...
#else
    float gety = smallsize * 1.4f + dtop +
                 (dheight - smallsize * .8f) * yh / (usedsize * 2.0f);
#endif

    if (portrait) {
      // Ensure gety (middle of text) is below status bar (dtop) plus half
      // headsize. Increased margin to ensure no overlap
      float safe_top = dtop + headsize + 20.0f * density;
      if (safe_top < 100.0f * density)
        safe_top = 100.0f * density;
    ...
    if (poll) {
      ...
      if (age < maxbluetoothage) {
        ...
        nvgRect(avg, getx + sensorbounds.left, sensorbounds.top + sensory,
      ...
      if (hist->notchinese()) {
        ...
        nvgText(avg, getx, gety, eusibinics.data(),
                eusibinics.data() + eusibinics.size());
        otherproblem = true;
      } else
#endif
      {
        ...
        LOGGER("waited=%lu warmup=%d starttime=%lu %s blueperm=%d\n", wait,
               warmup, starttime, ctime(&starttime), blueperm);
        ...
        if (bluescanner && blueperm < 2 && !hasnetwork()) {
          ...
          getboxwidth(usegetx);
          const std::string_view perm =
              blueperm == 1 ? usedtext->nolocationpermission
                            : usedtext->nonearbydevicespermission;
          const auto *bufptr = perm.data();
          const auto ends = perm.size();
          otherproblem = true;
          nvgTextBox(avg, usegetx, gety, getboxwidth(usegetx), bufptr,
                     bufptr + ends);
#ifndef DONTTALK
          ...
          if (wait < (warmup * 60) &&
              ((blueperm > 0 && !settings->data()->nobluetooth &&
                bluetoothEnabled()) ||
               hasnetwork())) {
            float usegetx = getx - headsize / 3;
            nvgTextAlign(avg, NVG_ALIGN_LEFT | NVG_ALIGN_MIDDLE);
            ...
            int ends;
            if ((hist->isSibionics() ||
                 (hist->isDexcom() && !hist->sensorerror))) {
              ...
              const bool isInitialised =
                  (!hist->isLibre2()) ||
                  sensors->getsensor(sensorindex)->initialized;
              LOGGER("wait<(%d*60) isInitialised=%d\n", warmup, isInitialised);
              static char buf[256];
              int minutes = warmup - (wait / 60);
              ends = sprintf(buf,
                             isInitialised ? usedtext->readysec.data()
                                           : usedtext->readysecEnable.data(),
                             minutes);
              bufptr = buf;
            }
            nvgTextBox(avg, usegetx, gety, getboxwidth(usegetx), bufptr,
            ...
            LOGAR("age>=maxbluetoothage");
            switch (showerrorvalue(avg, hist, nu, getx, gety,
                                   i)) { // TODO: integrate with same above
            case 1:
              neterror = true;
              break;
            case 2:
  ...
  if (!success && !otherproblem) {
    ...
    if (neterror) {
      //            nvgText(avg,newgetx ,gety, usedtext->networkproblem.begin(),
      //            usedtext->networkproblem.end());
      nvgTextBox(avg, newgetx, gety, getboxwidth(newgetx),
                 usedtext->networkproblem.begin(),
                 usedtext->networkproblem.end());
#ifndef DONTTALK
      shownglucose[i].glucosevalue = 0;
      ...
      if (usebluetoothoff) {
        ...
        nvgTextBox(avg, newgetx, gety, getboxwidth(newgetx),
                   usedtext->useBluetoothOff.begin(),
                   usedtext->useBluetoothOff.end());
#ifndef DONTTALK
        shownglucose[i].glucosevalue = 0;
        shownglucose[i].glucosevaluex = newgetx;
        shownglucose[i].glucosevaluey = gety + headsize * .5;
        ...
        if (blueperm < 1) {
          const std::string_view perm =
              blueperm == 1 ? usedtext->nolocationpermission
                            : usedtext->nonearbydevicespermission;
          nvgTextBox(avg, newgetx, gety, getboxwidth(newgetx), perm.begin(),
                     perm.end());
#ifndef DONTTALK
          shownglucose[i].glucosevalue = 0;
          shownglucose[i].glucosevaluex = newgetx;
          ...
          if (bluetoothoff) {
            nvgTextBox(avg, newgetx, gety, getboxwidth(newgetx),
                       usedtext->enablebluetooth.begin(),
                       usedtext->enablebluetooth.end());
#ifndef DONTTALK
            shownglucose[i].glucosevalue = 0;
            shownglucose[i].glucosevaluex = newgetx;
            shownglucose[i].glucosevaluey = gety + headsize * .5;

...
extern "C" JNIEXPORT void JNICALL fromjava(setCustomCalibrationSettings)(
  ...
  sistream *stream = reinterpret_cast<sistream *>(dataptr);
  auto *sens = stream->hist;
  sens->getinfo()->useCustomCalibration = enabled;
  sens->getinfo()->customCalIndex = index;
  sens->getinfo()->autoResetAlgorithm = autoReset;
}
...
fromjava(getCustomCalibrationSettings)(JNIEnv *env, jclass cl, jlong dataptr) {
  ...
  sistream *stream = reinterpret_cast<sistream *>(dataptr);
  auto *sens = stream->hist;
  // Pack return values into a long: [index:8][autoReset:1][active:1]
  long ret = 0;
  if (sens->getinfo()->useCustomCalibration)
    ret |= 1;
  if (sens->getinfo()->autoResetAlgorithm)
    ret |= 2;
  ret |= (long)sens->getinfo()->customCalIndex << 8;
  return ret;
}
...
extern "C" JNIEXPORT void JNICALL fromjava(siClearCalibration)(JNIEnv *env,
  ...
  if (!dataptr)
    return;
#ifdef SIBIONICS
  std::lock_guard<std::recursive_mutex> lock(si_lifecycle_mutex);
  if (active_streams.find((void *)dataptr) == active_streams.end())
    return;
#endif
  sistream *stream = reinterpret_cast<sistream *>(dataptr);
  auto *sens = stream->hist;
  if (sens) {
    // std::lock_guard<std::mutex> lock(stream->mutex); // Replaced global
...
SiContext::SiContext(SensorGlucoseData *sens)
    : binState(2, sens->binstatefile, 4096),
      ...
      notchinese(sens->notchinese()), algorithmUnsafe(false) {
  // CRITICAL: Detect corrupt state files on startup
  // If algcontext is null, the algorithm failed to initialize (corrupt state)
  // Wipe the state and retry initialization from scratch
  if (!algcontext) {
    LOGSTRING("SiContext constructor: algcontext is NULL! State files may be "
              "corrupt. Wiping and retrying...\n");

    // Reset/wipe the binState to clear corrupt data
    binState.reset();
    ...
    if (!algcontext) {
      LOGAR("SiContext constructor: FATAL - algcontext still NULL after wipe! "
            "Algorithm initialization completely failed.");
      // Mark as unsafe permanently - this prevents ANY processing attempts
      algorithmUnsafe = true;
    } else {
      LOGSTRING("SiContext constructor: Successfully recovered after wiping "
                "corrupt state\n");
    }
  }
}; // Start in safe state
...
void SiContext::reset(SensorGlucoseData *sens) {
  ...
  // CRITICAL: Wipe the entire binState memory to prevent algorithm from reading
  // old/corrupted data structures. Without this, initAlgorithm may access
  // invalid pointers and cause SIGSEGV.
  ...
  // Clear the reset flag to avoid triggering hardware reset.
  info->reset = false;

  // Save current siIndex. initAlgorithm checks (getSiIndex() > 1 &&
  // binState.get(0) == null) and calls resetSiIndex. Since we just cleared
  // binState, this would trigger duplicate reset! Solution: temporarily set
  // siIndex to 0 so initAlgorithm skips that check
  ...
  LOGGER("SiContext::reset() recreated algcontext. mNativeContext=%lld\n",
    ...
    // CRITICAL: Clear redoAll flag to allow immediate processing of new live data
    // Manual reset doesn't need history replay - user wants fresh start
    info->redoAll = false;
...
void SiContext::wipeDataOnly(SensorGlucoseData *sens) {
  ...
  unlink(sens->binstatefile.c_str());
  unlink(sens->statefile.c_str());
  unlink(pathconcat(sens->getsensordir(), "sta"
                                          "te3"
                                          ".js"
                                          "on")
             .c_str());
...
fromjava(SIprocessData)(JNIEnv *envin, jclass cl, jlong dataptr,
  ...
  std::lock_guard<std::recursive_mutex> lock(si_lifecycle_mutex);
  if (active_streams.find((void *)dataptr) == active_streams.end()) {
    LOGAR("SIprocessData REJECTED: invalid dataptr (UAF prevented)");
    return 0LL;
  }
#endif

  sistream *sdata = reinterpret_cast<sistream *>(dataptr);
  // std::lock_guard<std::mutex> lock(sdata->mutex); // Replaced by global lock
  ...
  if (sens->notchinese()) {
    ...
    // Auto-calibration reset: periodically reset calibration state without
    // resetting sensor
    if (info->useCustomCalibration && info->autoResetAlgorithm) {
      int resetHours =
          SensorGlucoseData::getCustomCalHours(info->customCalIndex);
      uint32_t lastReset = info->lastCalResetTime;
      if (lastReset == 0)
        lastReset = info->starttime; // First time: use sensor start
      uint32_t hoursSinceReset = (timsec - lastReset) / 3600;
      if (hoursSinceReset >= resetHours) {
        LOGGER("Auto Calibration Reset: %u hours since last reset (limit=%d)\n",
               hoursSinceReset, resetHours);
        sdata->sicontext.reset(sens);
        info->lastCalResetTime = timsec;
        // Also set redoAll to trigger full history replay
        info->redoAll = true;

        // CRITICAL: Don't process ANY data after reset! The new algcontext
        // hasn't processed history yet. Return immediately and let redoAll
        // handle replaying history on the next call.
        LOGSTRING("Auto reset complete - skipping current data, will replay on "
                  "next call\n");
        return 0LL;
      }
    }
    // Check if automatic time-based reset triggered history replay
    if (info->redoAll) {
      LOGSTRING("Skipping: auto-reset history replay pending\n");
      return 0LL;
    }

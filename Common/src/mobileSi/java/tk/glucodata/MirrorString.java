...
class MirrorString {
    ...
    /*
     * static String makestring(String[] names,int nr,boolean detect,String
     * port,boolean nums,boolean stream,boolean scans,boolean recover,boolean
     * receive,boolean activeonly,boolean passiveonly,String pass,long
     * starttime,String label,boolean testip,boolean hasname) throws JSONException {
    ...
    static void fromstring(String jsonstr, MainActivity act) {
        try {
            ...
            Runnable save = () -> {
                int pos = Natives.changebackuphost(-1, names, nr, detect, port, nums, stream, scans, false, receive,
                        activeonly, passiveonly, pass, starttime, label, testip, hasname, null, false);
                if (pos < 0) {
            ...
            if (receive) {
                ...
                if (type.length() > 0) {
                    Confirm.ask(act, act.getString(R.string.datapresent) + type, act.getString(R.string.overwrite),
                            save);
                    return;
                }
            }
